...
class BurgerConstructor extends Component<IProps> {
    ...
    componentDidMount() {
        if (!this.state.fixedBun) {
            this.setState((prevState) => ({ ...prevState, fixedBun: this.props.data[0] }))
        }
        if (this.state.selectedIngridient.length === 0 && this.selectedIngridientID.length > 0) {
            ...
            for (let i = 0; i < this.selectedIngridientID.length; i++) {
                const ingredient = this.props.data.find(el => el._id === this.selectedIngridientID[i]);
                if (ingredient) {
            ...
            this.setState((prevState) => ({...prevState, selectedIngridient: newSelectedIngredients}));
        }
    }
    ...
    render() {
        ...
        return <article className={`${burgerConstructorStyle.container} pt-25 pl-4 pr-4`} >
                ...
                <div className={burgerConstructorStyle.ingredients_inners}>
                    {
                        this.state.selectedIngridient
                            ...
                            .map(item => 
                                <section style={{ display: 'flex', alignItems: 'center', gap: '8px' }} className='pr-4' >
            ...
            <section className={`${ burgerConstructorStyle.payment } mt-10 pr-10`}>
                ...
                <div className={`${ burgerConstructorStyle.payment_currency } mr-10`}>
                    <p className='text text_type_digits-default pr-2'>
                        {this.state.selectedIngridient.reduce((summ, item) => summ + item.price, 0)}
                    </p>
